...
def find_peak(list_of_integers):
    ...
    # compara el izq  y der con pico y si es mas grande sera lo que retorna
    if peak_list > list_of_integers[left] and peak_list > list_of_integers[list_div + 1]:
        return peak_list
    # sino si el pico es menor a lo que hay a su izq hace recursiv
    elif peak_list < list_of_integers[left]:
        return find_peak(list_of_integers[:list_div])
    else: # si no es ninguna de las anteriores recursiv para la derecha del pico
